<html>
 <header style="background-color:#E9967A">  
 <a href="https://www.yesasia.ru">Information</a>
 </header>
 <main style="background-color:#FFDAB9">
    <h3 style="font-size:50px;background-color:#FFDAB9; text-align:center">К-поп гурти, що продали більше 1 мільйона копій альбомів у 2022 році 
</h3>
 </main>
    <img src="https://yesasia.b-cdn.net/wp-content/uploads/2022/10/1665093745-untitled-1-700x438.jpg"height=600px
    <p style="font-size:20px"><h4>К-поп та корейська культура продовжують захоплювати світ, що призводить до того, що зростає кількість покупців альбомів айдолів. Популярні виконавці легко продають понад 1 мільйон копій альбомів з кожним камбеком і список «груп-мільйонників» продовжує поповнюватися новими іменами. Цього року навіть дівочі гурти легше досягають цього числа. 

Отже, ось артисти, які продали понад 1 мільйон копій у 2022 році!</h4> </p>
<h3> 1:BTS</h3> <img src="https://play-lh.googleusercontent.com/hGy6TRJW6WUBQIdJvhThkQrB62ZYrCh1lahvGbBnc9ap5caelQAETF8qcG9lu_fHi_GvT6b1ZA=w3840-h2160-rw" height="550px"/>
 <h3>2:BLACKPINK </h3> <img src="https://6.viki.io/image/4c6acf107c2745e88579ef59cdef5ec4/dummy.jpeg?s=900x600&e=t" height="550px"/>
 <h3> 3: aespa </h3><img src="https://www.nme.com/wp-content/uploads/2022/07/aespa-girls-concept-photo-released-later.jpg" height="400px"/>
 <h3> 4: Seventeen </h3><img src="https://www.allkpop.com/upload/2022/04/content/302047/1651366075-1650340978027-svt-jp-hanabi-pc2.jpg" height="550px"/>
 <h3> 5: TWICE </h3> <img src="https://6.viki.io/image/43c2bf6dad174557b459f1250f04672d/dummy.jpeg?s=900x600&e=t" height="550px"/>
 <h3> 6: Stray Kids </h3> <img src="https://lastfm.freetls.fastly.net/i/u/770x0/40896193dd916574187a4c2363c0ce86.jpg" height="550px"/>
 <h3> 7: Ateez </h3> <img src="https://cdn.shopify.com/s/files/1/0469/3927/5428/articles/ateez-ihr-weg-zum-debut-und-in-die-herzen-der-fans-603519.jpg?v=1629738316" height="550px"/>
 <h3> 8: ITZY </h3> <img src="https://www.allkpop.com/upload/2022/08/content/051051/web_data/allkpop_1659712519_untitled-1.jpg" height="550px"/>
 <h3> 9: Enhypen </h3> <img src="https://assets.teenvogue.com/photos/5fd6e22cd0633f215113055d/master/pass/ENHYPEN_DUSK_Photo%20Courtesy%20BELIFT%20LAB_1.jpg" height="550px"/>
 <h3> 10: IVE </h3> <img src="https://www.rappler.com/tachyon/2022/07/ive-comeback.png" height="550px"/>
     <table style="border:2px solid #FF6347;font-size:50px">
        <tr>
            <td style="border:3px solid #FF6347;">K-pop гурт:</td>
            <td style="border:3px solid #FF6347;">Альбом:</td>
            <td style="border:3px solid #FF6347;">Кількість проданих копій:</td>
        
        </tr>
        <tr>
            <td style="border:3px solid #FF6347;">ВTS</td>
            <td style="border:3px solid #FF6347;">Proof</td>
            <td style="border:3px solid #FF6347;">2,75M</td>
         </tr>
          <tr>
            <td style="border:3px solid #FF6347;">BLACKPINK</td>
            <td style="border:3px solid #FF6347;">BORN PINK</td>
            <td style="border:3px solid #FF6347;">2,51M</td>
        </tr>
         <tr>
            <td style="border:3px solid #FF6347;">aespa</td>
            <td style="border:3px solid #FF6347;">GIRLS</td>
            <td style="border:3px solid #FF6347;">1,65M</td>
        </tr>
         <tr>
            <td style="border:3px solid #FF6347;">Seventeen</td>
            <td style="border:3px solid #FF6347;">Face the Sun</td>
            <td style="border:3px solid #FF6347;">2,72M</td>
        </tr>
         <tr>
            <td style="border:3px solid #FF6347;">TWICE</td>
            <td style="border:3px solid #FF6347;">Between 1&2</td>
            <td style="border:3px solid #FF6347;">1,6M</td>
        </tr>
         <tr>
            <td style="border:3px solid #FF6347;">Stray Kids</td>
            <td style="border:3px solid #FF6347;">Maxident</td>
            <td style="border:3px solid #FF6347;">1,3M</td>
        </tr>
         <tr>
            <td style="border:3px solid #FF6347;">Ateez</td>
            <td style="border:3px solid #FF6347;">Movement</td>
            <td style="border:3px solid #FF6347;">1M</td>
        </tr>
         <tr>
            <td style="border:3px solid #FF6347;">ITZY</td>
            <td style="border:3px solid #FF6347;">CHECKMATE</td>
            <td style="border:3px solid #FF6347;">1,1M</td>
        </tr>
         <tr>
            <td style="border:3px solid #FF6347;">Enhypen</td>
            <td style="border:3px solid #FF6347;">Manifesto: Day 1</td>
            <td style="border:3px solid #FF6347;">1,3M</td>
        </tr>
         <tr>
            <td style="border:3px solid #FF6347;">IVE</td>
            <td style="border:3px solid #FF6347;">After like</td>
            <td style="border:3px solid #FF6347;">1,1M</td>
        </tr>
     </table> 
     <footer style="background-color:#E9967A;color:black;">
        <p style="font-size:15px"> Автор: katelvii </p>
     </footer>
</html>
